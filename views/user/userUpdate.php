...
?>
<?php
if (isset($_GET['prodID']) && isset($_GET['cartID'])) {
    ...
    $query2 = "SELECT * FROM `cart` WHERE cartID='$cartid'";

    $cartDetails = mysqli_query($conn, $query2);

    $row2 = mysqli_fetch_assoc($cartDetails);
                    ...
                    ?>
                            ...
                            <div class="p-4">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-12 col-sm-4">
                                        <div class="form-group">
                                            <label for="prodInv">Maximum Remaining</label>
                                            <input readonly type="number" min="0" oninput="this.value = !!this.value && Math.abs(this.value) >= 0 ? Math.abs(this.value) : null" class="form-control inactiveLink input-update" required name="max" id="max" value="<?=$max?>">
                                   ...
                                   <div class="col-12 col-sm-6 text-center" >
                                        <div class="form-group">
                                            <label for="prodQuantity">Current Quantity</label>
                                            <input type="number" min="1" step="1"  oninput="" title="Numbers only" readonly onkeypress="return event.charCode >= 48 && event.charCode <= 57" max="<?=$max?>" class="form-control text-center input-update" required name="currqty" id="currqty" value="<?=$quantity?>">
                                        </div>
                                    </div>

                                   <div class="col-12 col-sm-6 text-center" >
                                        <div class="form-group">
                                            <label for="prodQuantity">Update Your Quantity</label>
                                            <input type="number" min="1" step="1"  oninput="" title="Numbers only" onkeypress="return event.charCode >= 48 && event.charCode <= 57" max="<?=$max?>" class="form-control text-center input-update" required name="prodQuantity" id="prodQuantity" value="<?=$quantity?>">
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-12 col-sm-4 text-center">
                                        <div class="form-group">
                                            ...
                                            <input type="text" readonly style="background-color:white;" class="form-control text-center input-update-2" required name="singleUnit" id="singleUnit" value="RM<?=$price?>">
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-12 col-sm-5 text-center" style="padding: 0% 5% ;">
                                        <div class="form-group">
                                            ...
                                            <input type="text"  step=".01" style="background-color:white;" readonly class="form-control text-center  input-update-2" required name="prodTotal" id="prodTotal" value="RM<?=$total?>">
                                        </div>
                                    </div>

                                    <input readonly type="hidden" name="productID" value="<?=$productID;?>">
                                    <input readonly type="hidden" name="vendorID" value="<?=$vendorID;?>">
                                    <input readonly type="hidden" name="prodInv" id="prodInv" value="<?=$inventoryNo?>">
                                    <input readonly type="hidden" name="originalQty" value="<?=$quantity;?>">
                                    <input readonly type="hidden" id="hiddenUnit" name="hiddenUnit" value="<?=$price?>">
                                    <input readonly type="hidden" id="hiddenTotal" name="hiddenTotal" value="<?=$total?>">

                                    <div class="col-12 text-right col-sm-3 " style="padding: 0% 5% 0% 0%; margin:4% 0% 0% 0%;">
                                        <button type="submit" name="updateCart"  class="btn" style="background-color:#007dd6; color:white;">Update Order</button>
                                    </div>
...
if (isset($_POST['updateCart'])) {
    ...
    if($prodQuantity == $originalQty){
        $stockLeft = $prodInv ;
    }
    elseif($prodQuantity > $originalQty)
    {
       $tolakStock = $prodQuantity - $originalQty;
       $stockLeft = $prodInv - $tolakStock;

    } elseif ($prodQuantity < $originalQty){

        $tambahStock = $originalQty - $prodQuantity;
        $stockLeft = $prodInv + $tambahStock;
    }
    ...
    `discount`='$prodDisc',
    `total`='$hiddenTotal' WHERE cartID='$cartID'";
